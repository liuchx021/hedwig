...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResult<Void>> handleBusinessException(BusinessException e) {
    ...
    return ResponseEntity.status(ec.getHttpStatus()).body(ApiResult.fail(ec, e.getMessage()));
  }
  ...
  private ResponseEntity<ApiResult<Void>> buildResponse(ErrorCode ec) {
    return ResponseEntity.status(ec.getHttpStatus()).body(ApiResult.fail(ec));
  }

  private ResponseEntity<ApiResult<Void>> buildResponse(ErrorCode ec, String message) {
    return ResponseEntity.status(ec.getHttpStatus()).body(ApiResult.fail(ec, message));
  }
}
