 ...
 *
 * <p>Base URL: https://api.ottai.com Auth: Bearer token (JWT HS512, valid 180 days) Token
 * validation: parse JWT payload Base64, extract "exp" field
 */
@Slf4j
@Component
public class OttaiClient implements VendorClient {
  ...
  public VendorTokenInfo validateToken(String accessToken) {
    ...
    log.info(
        "[Ottai] validateToken called, token length={}",
    ...
    try {
      ...
      log.info(
          "[Ottai] Token validation: userId={}, expiresAt={}, valid={}", userId, expiresAt, valid);
  ...
  /**
   * Ottai login requires WeChat mini-program codes and cannot be automated server-side. This method
   * is provided for interface compliance only.
   */
  @Override
  public VendorTokenInfo login(VendorLoginRequest loginRequest) {
    throw new VendorException("欧泰暂不支持通过账号密码直接登录，请在微信小程序中抓包获取访问令牌后再连接。");
  }
    ...
    try {
      ...
      for (int i = 0; i < data.size(); i++) {
        ...
        String subjectUserId =
            item.containsKey("fromUserId") ? String.valueOf(item.getLongValue("fromUserId")) : null;
        String deviceId =
            item.containsKey("fromUserDeviceId")
                ? String.valueOf(item.getLongValue("fromUserDeviceId"))
                : null;
        String displayName =
            item.containsKey("fromUserRemark") ? item.getString("fromUserRemark") : null;
  ...
  /**
   * Lightweight realtime fetch: one POST to /relatives returns latest glucose, receiveTime, and
   * arrowType for ALL monitored subjects.
   *
  ...
  public Map<String, VendorGlucoseData> getRealtimeFromRelatives(
    ...
    try {
      ...
      for (int i = 0; i < data.size(); i++) {
        ...
        if (!item.containsKey("glucose")
            || item.get("glucose") == null
            || !item.containsKey("receiveTime")
            || item.get("receiveTime") == null) {
          continue;
        }
        ...
        try {
          ...
          log.warn(
              "[Ottai] Skipping subject {} with unparseable glucose: {}",
              subjectId,
              item.getString("glucose"));
          continue;
        }
  ...
  public List<VendorGlucoseData> getHistoricalGlucose(
      String accessToken, String vendorUserId, VendorSubject subject) {
    log.info(
        "[Ottai] getHistoricalGlucose: vendorUserId={}, subjectId={}, deviceId={}",
        vendorUserId,
        subject.getSubjectId(),
        subject.getDeviceId());
    try {
      log.info(
          "[Ottai] GET {}/link/application/server/tag/search/queryMonitorBase?deviceId={}&userId={}&isOpen=1&timeType=2&unit=mmol_L",
          BASE_URL,
          subject.getDeviceId(),
          subject.getSubjectId());
      String body =
      ...
      JSONObject response = JSON.parseObject(body);
      log.info(
          "[Ottai] getHistoricalGlucose response code: {}",
  ...
  private void buildHeaders(
      org.springframework.http.HttpHeaders headers, String accessToken, String vendorUserId) {
    String normalizedToken = VendorTokenNormalizer.normalize(accessToken);
  ...
  private VendorException apiError(String operation, WebClientResponseException e) {
    ...
    String message = String.format("欧泰接口请求失败（%s，HTTP %s）", operation, e.getStatusCode().value());
