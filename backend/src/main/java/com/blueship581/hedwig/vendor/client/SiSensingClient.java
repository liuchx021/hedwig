...
import com.blueship581.hedwig.vendor.client.dto.SiSensingApiResponse;
import com.blueship581.hedwig.vendor.client.dto.SiSensingLoginPayload;
...
public class SiSensingClient implements VendorClient {
  ...
  public VendorTokenInfo validateToken(String accessToken) {
    ...
    try {
      ...
      SiSensingApiResponse response = JSON.parseObject(body, SiSensingApiResponse.class);
      if (response == null || response.isFailed()) {
        return VendorTokenInfo.builder().valid(false).build();
      }

      JSONObject tokenInfo = response.getDataAsObject();
      if (tokenInfo == null) {
        return VendorTokenInfo.builder().valid(false).build();
      }
  ...
  public VendorTokenInfo login(VendorLoginRequest loginRequest) {
    ...
    try {
      String payload =
          JSON.toJSONString(
              SiSensingLoginPayload.builder().phone(phone).password(password).build());
      ...
      if (response.isFailed()) {
        String detail = response.getErrorDetail();
        throw new VendorException(
            "硅基轻享账号密码登录失败" + (detail != null && !detail.isBlank() ? "：" + detail : ""));
      }

      // data 字段可能是加密字符串，尝试解密
      Object dataRaw = response.getData();
      ...
      String token =
          data != null
              ? firstText(
                  data, "token", "accessToken", "access_token", "authorization", "bearerToken")
              : null;
  ...
  public List<VendorSubject> getMonitoredSubjects(String accessToken, String vendorUserId) {
    try {
      ...
      if (response.isFailed()) {
        String detail = response.getErrorDetail();
        throw new VendorException(
            "硅基轻享接口返回异常：获取监测对象列表" + (detail != null && !detail.isBlank() ? "：" + detail : ""));
      }

      JSONObject payload = response.getDataAsObject();
  ...
  public List<VendorGlucoseData> getHistoricalGlucose(
    ...
    try {
      ...
      log.debug("[SiSensing] getHistoricalGlucose raw response: {}", body);
      SiSensingApiResponse response = JSON.parseObject(body, SiSensingApiResponse.class);
      if (response == null) {
      ...
      if (response.isFailed()) {
        String detail = response.getErrorDetail();
        throw new VendorException(
            "硅基轻享接口返回异常：获取历史血糖数据" + (detail != null && !detail.isBlank() ? "：" + detail : ""));
      }
      ...
      log.error("[SiSensing] getHistoricalGlucose WebClientResponseException HTTP={} body={}",
          e.getStatusCode().value(), e.getResponseBodyAsString(), e);
      throw apiError("获取历史血糖数据", e);
    } catch (Exception e) {
      log.error("[SiSensing] getHistoricalGlucose unexpected error", e);
  ...
  private VendorException apiError(String operation, WebClientResponseException e) {
    ...
    return new VendorException(
        detail != null && !detail.isBlank() ? message + "：" + detail : message, e);
  }

  private String extractErrorDetail(String body) {
    if (body == null || body.isBlank()) return null;
    try {
      SiSensingApiResponse parsed = JSON.parseObject(body, SiSensingApiResponse.class);
      String detail = parsed.getErrorDetail();
