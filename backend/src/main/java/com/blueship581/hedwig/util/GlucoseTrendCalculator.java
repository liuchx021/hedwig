...
public final class GlucoseTrendCalculator {
    ...
    /**
     * Expected interval between two consecutive CGM readings in seconds.
     * Most CGM sensors produce a reading every 5 minutes (300s).
     */
    private static final double EXPECTED_INTERVAL_SEC = 300.0;

    /**
     * Maximum interval (in seconds) beyond which trend calculation is not meaningful.
     * If two readings are more than 30 minutes apart, the rate-of-change is unreliable.
     */
    ...
    /**
     * Calculate trend direction from two consecutive readings, normalizing by time
     * interval. Falls back to simple delta when timestamps are unavailable.
     *
    ...
    public static TrendDirection calculate(Double currentMmol, Double previousMmol,
                                           Instant currentTime, Instant previousTime) {
    ...
    /**
     * Backward-compatible overload: calculate without timestamps.
     * Assumes the two readings are exactly one standard interval apart.
     */
    public static TrendDirection calculate(Double currentMmol, Double previousMmol) {
