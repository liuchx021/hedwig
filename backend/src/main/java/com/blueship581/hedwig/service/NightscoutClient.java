...
/**
 * Client for pushing glucose data to a Nightscout server. Supports both dynamic targets (from DB)
 * and legacy config-based mode.
 */
@Slf4j
@Component
public class NightscoutClient {
  ...
  /**
   * Push entries to a specific Nightscout target (DB-driven). Returns the number of successfully
   * pushed entries.
   */
  public int pushEntries(NightscoutTarget target, List<SgvEntry> entries) {
    try {
      ...
      log.info(
          "Pushed {} entries to Nightscout target '{}' ({})",
          entries.size(),
          target.getName(),
          target.getBaseUrl());
      return entries.size();
      ...
      log.error(
          "Nightscout push failed for target '{}' with status {}: {}",
          target.getName(),
          e.getStatusCode(),
  ...
  public boolean testConnection(NightscoutTarget target) {
    try {
      ...
      log.warn(
          "Nightscout connection test failed for target '{}': {}",
          target.getName(),
          e.getMessage());
      return false;
    }
  ...
  /** Build an SgvEntry from mmol/L glucose value. Converts mmol to mg/dL internally. */
  public SgvEntry toSgvEntry(
      double glucoseMmol, Instant readingTime, String direction, String device) {
  ...
  public SgvEntry toSgvEntryFromMgdl(
      double glucoseMgdl, Instant readingTime, String direction, String device) {
  ...
  private SgvEntry buildSgvEntry(int mgdl, Instant readingTime, String direction, String device) {
    String dateStr =
        DateTimeFormatter.ISO_OFFSET_DATE_TIME.withZone(ZoneId.of(timezone)).format(readingTime);
  ...
  public static class SgvEntry {
    @Builder.Default private String type = "sgv";
    private int sgv;
    private long date;
